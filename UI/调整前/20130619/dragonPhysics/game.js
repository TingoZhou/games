...
var STATE_VICTORY=5;
var STATE_LOOSE=6;
var STATE_END_GAME=7;
var STATE_PREPARE_LEVEL=100;
...
function startLoad() {
    ...
    if (GET["debug"] != 1)Utils.checkOrientation(false);
    ...
    data.push({name:"blank", src:"images/blank.gif"});
    data.push({name:"back", src:path + "back.jpg"});
    data.push({name:"back_game", src:path + "back_game.jpg"});
    data.push({name:"back_select_level", src:path + "back_select_level.jpg"});
    data.push({name:"level_completed", src:path + "level_completed.png"});
    data.push({name:"level_current", src:path + "level_current.png"});
    data.push({name:"level_unavailable", src:path + "level_unavailable.png"});
    data.push({name:"button_play", src:path + "button_play.png"});
    data.push({name:"button_highscores", src:path + "button_highscores.png"});
    data.push({name:"button_restart", src:path + "button_restart.png"});
    data.push({name:"button_next", src:path + "button_next.png"});
    data.push({name:"button_end", src:path + "button_end.png"});
    data.push({name:"button_submit", src:path + "button_submit.png"});
    data.push({name:"button_menu_mini", src:path + "button_menu_mini.png"});
    data.push({name:"button_restart_mini", src:path + "button_restart_mini.png"});
    data.push({name:"pers_birth", src:path + "pers_birth.png"});
    data.push({name:"hourglass", src:path + "hourglass.png"});
    data.push({name:"text1", src:path + "text1.png"});
    data.push({name:"font1", src:path + "font1.png"});
    data.push({name:"help", src:path + "help.png"});
    data.push({name:"popup_victory", src:path + "popup_victory.png"});
    data.push({name:"popup_loose", src:path + "popup_loose.png"});
    data.push({name:"check", src:path + "check.png"});
    data.push({name:"check2", src:path + "check2.png"});
    data.push({name:"logo", src:path + "logo_spil.jpg"});
...
function createScene() {
    ...
    if (gameState == STATE_MENU) {
        ...
        mc.static=true;
        stage.addChild(mc);
        mc=new Sprite(bitmaps.button_play, 168, 42, 1);
        mc.x=160;
        mc.y=310;
        mc.onclick=showSelectLevel;
        stage.addChild(mc);
        mc=new Sprite(bitmaps.button_highscores, 168, 42, 1);
        mc.x=160;
        mc.y=360;
    ...
    if (gameState == STATE_SELECT_LEVEL) {
        ...
        mc.static=true;
        stage.addChild(mc);
        mc=new Sprite(bitmaps.button_menu_mini, 38, 38, 1);
        mc.x=290;
        mc.y=30;
        mc.static=true;
        mc.onclick=showMenu;
        ...
        var lastLevel=-1;
        for (var i=0; i < 20; i++) {
            if (levelsScores[i] >= 0)lastLevel=i;
        }
        lastLevel++;
        var bm, dc;
        for (var i=0; i < 20; i++) {
            if (i == lastLevel)bm=bitmaps.level_current;
            if (i < lastLevel)bm=bitmaps.level_completed;
            if (i > lastLevel)bm=bitmaps.level_unavailable;
            mc=new Sprite(bm, 50, 65, 1);
            ...
            if (i < lastLevel)drawText(levelsScores[i], mc.x, mc.y);
        }
    }
    buildBackground();
    stage.start();
...
function getLevelsScores() {
    levelsScores=[];
    var s=Utils.getCookie('remover1_levels_scores') + "";
    if (s != "null") {
...
function saveLevelsScores() {
    ...
    Utils.setCookie('remover1_levels_scores', s);
}
...
function startMain(e) {
    if (e.target.opacity < 1)e.target.opacity+=0.05;
...
function constructLevel(n) {
    if (gameState == STATE_PREPARE_LEVEL)return;
    if (stage) {
    ...
    setTimeout("startLevel(" + n + ")", 1000 / fps * 2);
}
function startLevel(n, data) {
    clearTimeout(tmLowScore);
    if (gameState == STATE_PREPARE_LEVEL)return;
    gameState=STATE_PREPARE_LEVEL;
    ...
    mc.static=true;
    stage.addChild(mc);
    mc=new Sprite(bitmaps.button_restart_mini, 38, 37, 1);
    mc.x=30;
    mc.y=30;
    mc.onclick=restartLevel;
    stage.addChild(mc);
    mc.static=true;
    mc=new Sprite(bitmaps.button_menu_mini, 38, 37, 1);
    mc.x=290;
    mc.y=30;
    mc.onclick=function () {
    ...
    stage.addChild(mc);
    mc.static=true;
    mc=new Sprite(bitmaps.text1, 89, 20, 1);
    mc.x=140;
    mc.y=30;
    stage.addChild(mc);
    mc.static=true;
    if (n == 0 && GET["debug"] != 1) {
        mc=new Sprite(bitmaps.help, 245, 282, 1);
        mc.x=195;
        mc.y=210;
        stage.addChild(mc);
        mc.static=true;
    ...
    scoresMc=[];
    for (var i=0; i < 3; i++) {
        mc=new Sprite(bitmaps.font1, 17, 19, 10);
        mc.stop();
        mc.visible=false;
        stage.addChild(mc);
        scoresMc.push(mc);
    ...
    else levelObjs=levels[n].objects;
    var lo, ob, body, points, density, restitution, friction;
    for (var i=0; i < levelObjs.length; i++) {
        ...
        if (ob.bodyType == 1)body=box2DCreateBox(world, lo.x, lo.y, ob.width / 2, ob.height / 2, lo.rotation, ob.fixed, density, restitution, friction);
        if (ob.bodyType == 2)body=box2DCreateCircle(world, lo.x, lo.y, ob.width / 2, lo.rotation, ob.fixed, density, restitution, friction);
        if (ob.bodyType == 3)body=box2DCreatePoly(world, lo.x, lo.y, ob.points, lo.rotation, ob.fixed, density, restitution, friction);
        if (ob.joints) {
        ...
        if (GET["debug"] != 1 && ob.fixed && ob.frames == 1)mc.static=true;
        if (ob.animDelay)mc.animDelay=ob.animDelay;
        if (ob.type == 2)base=mc;
        if (ob.type == 1)ball=mc;
    }
    checkState=false;
    checkBallState=0;
    curScore=100;
...
function drawDigits(val, mc, x, y) {
    var s="" + val;
    var o;
    var sX=x;
...
function drawScore() {
    for (var i=0; i < 3; i++)scoresMc[i].visible=false;
    drawDigits(curScore > 99 ? 99 : curScore, scoresMc, 200, 30);
}
var tmLowScore;
function lowScore() {
    clearTimeout(tmLowScore);
    curScore--;
    if (curScore > 0)tmLowScore=setTimeout(lowScore, 1000);
}
...
function removeObject(e) {
    if (checkState || gameState != STATE_GAME)return;
...
function addCheck(ball) {
    if (!ball)checkState=true;
    check=new Sprite((ball ? bitmaps.check : bitmaps.check2), 23, 23, 5);
    check.stop();
    check.x=base.x;
    check.y=base.y;
    check.ball=ball;
...
function changeCheck() {
    ...
    if (check.currentFrame >= 5) {
        check.destroy=true;
        if (check.ball) {
            if (checkBallState == 1)checkBallState=2;
        }
        else checkState=false;
    }
    else setTimeout(changeCheck, 250);
}
function showLoose() {
    clearTimeout(tmLowScore);
    gameState=STATE_LOOSE;
    mc=new Sprite(bitmaps.popup_loose, 300, 285, 1);
    mc.x=160;
    mc.y=200;
    stage.addChild(mc);
    mc=new Sprite(bitmaps.button_restart, 168, 41, 1);
    mc.x=160;
    mc.y=352;
    mc.onclick=restartLevel;
    stage.addChild(mc);
...
function showVictory() {
    clearTimeout(tmLowScore);
    if (levelsScores[curLevel] < curScore) {
        levelsScores[curLevel]=curScore;
        saveLevelsScores();
    }
    submitScores();
    gameState=STATE_VICTORY;
    mc=new Sprite(bitmaps.popup_victory, 300, 291, 1);
    mc.x=160;
    mc.y=160;
    stage.addChild(mc);
    if (curLevel < 19) {
        mc=new Sprite(bitmaps.button_next, 168, 41, 1);
        mc.onclick=nextLevel;
    }
    else {
        mc=new Sprite(bitmaps.button_end, 168, 41, 1);
        mc.onclick=showMenu;
    }
    mc.x=160;
    mc.y=315;
    stage.addChild(mc);
    drawText(curScore, 160, 200);
    drawText(getTotalLevelsScores(), 160, 262);
}
function nextLevel() {
    curLevel++;
...
function completeLevel() {
    gameState=STATE_VICTORY;
    mc=new Sprite(bitmaps.pers_birth, 75, 38, 15);
    mc.x=ball.x;
    mc.y=ball.y;
...
function mainLoop() {
    if (!ball || !base || gameState != STATE_GAME)return;
    world.Step(1 / (fps * 2), 1);
    world.Step(1 / (fps * 2), 1);
    stage.box2dSync(world);
    if (ball.y > 500 || ball.x < -40 || ball.x > 400) {
        showLoose();
    }
    if (!checkState) {
        for (var b=ball.box2dBody.GetContactList(); b; b=b.next) {
            if (b.other.sprite == base) {
                if (checkBallState == 2) {
                    completeLevel();
                    return;
                }
...
var ExternalAPI={type:"Spilgames", init:function (callback) {
    if (typeof SpilGames == 'undefined') {
        if (typeof window.parent != 'undefined') {
            SpilGames=window.parent.SpilGames;
        }
    }
    if (ExternalAPI.check())SpilGames.Events.subscribe('gamecontainer.resize', Utils.fitLayoutToScreen); else Utils.addFitLayoutListeners();
}, check:function () {
    return(typeof SpilGames != 'undefined');
}, checkUserLoggedIn:function () {
    var state=SpilGames.Auth.getAuthState();
    if (state == "NOT_AUTHENTICATED")return false;
    return true;
    ...
    SpilGames.Highscores.insert({score:val}, callback);
}}
...
var Utils={touchScreen:false, globalScale:1, setCookie:function (name, value) {
    ...
    return get;
}, globalPixelScale:1, getMouseCoord:function (event, object) {
    ...
    var retX=(mouseX - x);
    var retY=(mouseY - y);
    return{x:retX / Utils.globalPixelScale, y:retY / Utils.globalPixelScale};
    ...
    if (Utils.touchScreen) {
        ...
        if (w > 480)scale=2;
        if (Utils.mobileCheckIphone4())scale=2;
    }
    else {
        if (landscape) {
            if (h >= 640)scale=2;
...
}, imagesRoot:'images', createLayout:function (container, resolution) {
    var scl=Utils.globalScale;
    var height=window.innerHeight;
    if ("orientation"in window)height=1024; else document.body.style.overflow="hidden";
    var s="";
    ...
    s+='<canvas id="screen" width="' + resolution.width + '" height="' + resolution.height + '">您的浏览器不支持这个应用(</canvas>';
    s+='</div>';
...
}, addMobileListeners:function (landscape, ignoreHTC, noLockScreen) {
    if (!noLockScreen) {
        if (navigator.userAgent.indexOf('Desire') == -1 || ignoreHTC) {
    ...
    var orient=(Math.abs(orientation) == 90);
    ...
    window.addEventListener("resize", function () {
        setTimeout(Utils.fitLayoutToScreen, 300);
    }, false);
    ...
    if (landscape)stage.drawRectangle(240, 275, 480, 90, "#f00", true, 1, true); else stage.drawRectangle(160, 435, 320, 90, "#f00", true, 1, true);
    ...
    if (Utils.globalScale == 1.5) {
        if (landscape)stage.drawRectangle(510, 160, 60, 320, "#000", true, 1, true); else stage.drawRectangle(160, 510, 320, 60, "#000", true, 1, true);
    }
}, grad2radian:function (val) {
    return val / (180 / Math.PI);
}, radian2grad:function (val) {
    return val * (180 / Math.PI);
}, eventsListeners:[], onlockscreen:null, onunlockscreen:null, onfitlayout:null, addEventListener:function (type, callback) {
...
function Sprite(img, w, h, f, l) {
    ...
    this.moveTo=function (x, y, duration, ease, onfinish, onchange) {
        ...
        if (duration <= 0) {
            ...
            var t;
            t=this.addTween('x', x, duration, ease, onfinish, onchange);
            if (t)t.play();
            t=this.addTween('y', y, duration, ease, null, null);
            if (t)t.play();
        }
        return this;
    }
    ...
    this.history={created:false, drawed:false, changed:false, x:0, y:0, rotation:0, frame:0, AABB:[]};
...
function Tween(obj, prop, start, end, duration, callback) {
    ...
    this.tick=function () {
        ...
        self.dispatchEvent("change", {target:self, value:val});
        return false;
    }
    this.finish=function () {
        self.stop();
        self.updateValue(self.end);
        return self.dispatchEvent("finish", {target:self, value:self.end});
    }
    this.eventsListeners=[];
    this.addEventListener=function (type, callback) {
        EventsManager.addEvent(this, type, callback);
...
var Easing={back:{easeIn:function (t, b, c, d) {
    ...
    if (t < d / 2)return Easing.bounce.easeIn(t * 2, 0, c, d) * 0.5 + b; else return Easing.bounce.easeOut(t * 2 - d, 0, c, d) * 0.5 + c * 0.5 + b;
...
function Stage(cnsId, w, h) {
    ...
    this.destroy=function () {
        ...
        this.stop();
        this.clear();
        this.canvas.ctx.clearRect(0, 0, this.screenWidth * Utils.globalScale * Utils.globalPixelScale, this.screenHeight * Utils.globalScale * Utils.globalPixelScale);
    }
    ...
    this.addChild=function (item) {
        ...
        var z=item.zIndex;
        if (f.index !== false)item.zIndex=f.zIndex + 1; else item.zIndex=0;
        this.objectsCounter++;
    ...
    this.getObjectsStackByCoord=function (x, y, pixelCheck, includeDragged) {
        var obj, cX, cY, r, present, imageData;
        var tmp=[];
        for (var i=0; i < this.objects.length; i++) {
            if (this.objects[i].visible || this.objects[i].eventsWhenInvisible) {
                obj=this.objects[i];
                cX=obj.getX() - obj.getWidth() / 2;
                cY=obj.getY() - obj.getHeight() / 2;
                ...
                present=false;
                if (obj.rotation == 0) {
                    if (cX <= x && cY <= y && cX + obj.getWidth() >= x && cY + obj.getHeight() >= y)present=true;
                }
                else {
                    r=new Rectangle(obj.getX(), obj.getY(), obj.getWidth(), obj.getHeight(), obj.rotation);
                    if (r.hitTestPoint(new Vector(x, y)))present=true;
                }
                if (present && pixelCheck) {
                    ...
                    this.buffer.ctx.clearRect(0, 0, this.screenWidth * Utils.globalScale * Utils.globalPixelScale, this.screenHeight * Utils.globalScale * Utils.globalPixelScale);
                    ...
                    imageData=this.buffer.ctx.getImageData(Math.floor(pX * Utils.globalPixelScale), Math.floor(pY * Utils.globalPixelScale), 1, 1);
                    if (imageData.data[3] == 0)present=false;
                }
                if (!present && includeDragged && obj.dragged)present=true;
                if (present)tmp.push(obj);
                if (obj.ignoreViewport) {
                    x+=this.viewport.x;
                    y+=this.viewport.y;
                }
    ...
    this.finalizeMouseCoords=function (obj, m) {
        ...
        eX=(eX - obj.getX()) / Utils.globalScale;
        eY=(eY - obj.getY()) / Utils.globalScale;
        return{x:eX, y:eY};
    }
    this.checkClick=function (event) {
        ...
        m.x+=this.viewport.x;
        m.y+=this.viewport.y;
        var stack=this.getObjectsStackByCoord(m.x, m.y, this.pixelClickEvent);
        var ret, f;
        if (stack.length > 0) {
            stack=this.sortStack(stack);
            for (var i=0; i < stack.length; i++) {
                f=this.finalizeMouseCoords(stack[i], m);
                ret=stack[i].dispatchEvent("click", {target:stack[i], x:f.x, y:f.y, raw:raw});
                if (ret === false)return;
            }
        }
    };
    this.checkContextMenu=function (event) {
        ...
        if (stack.length > 0) {
            ...
            for (var i=0; i < stack.length; i++) {
                f=this.finalizeMouseCoords(stack[i], m);
                ret=stack[i].dispatchEvent("contextmenu", {target:stack[i], x:f.x, y:f.y, raw:raw});
                if (ret === false)return;
            }
        }
    };
    this.checkMouseMove=function (event) {
        ...
        for (i=0; i < this.objects.length; i++) {
            if (this.objects[i].dragged) {
                ...
                this.objects[i].x=eX / Utils.globalScale - this.objects[i].dragX;
                this.objects[i].y=eY / Utils.globalScale - this.objects[i].dragY;
            }
        }
        var stack=this.getObjectsStackByCoord(m.x, m.y, this.pixelMouseMoveEvent);
        var i, n, ret, bOk, f;
        var overStack=[];
        if (stack.length > 0) {
            stack=this.sortStack(stack);
            for (i=0; i < stack.length; i++) {
                ...
                f=this.finalizeMouseCoords(stack[i], m);
                if (!stack[i].mouseOn)ret=stack[i].dispatchEvent("mouseover", {target:stack[i], x:f.x, y:f.y, raw:raw});
                stack[i].mouseOn=true;
                if (ret === false)break;
            }
            for (i=0; i < stack.length; i++) {
                f=this.finalizeMouseCoords(stack[i], m);
                ret=stack[i].dispatchEvent("mousemove", {target:stack[i], x:f.x, y:f.y, raw:raw});
                if (ret === false)break;
            }
        }
        for (i=0; i < this.objects.length; i++) {
            if (this.objects[i].mouseOn) {
                ...
                if (!bOk) {
                    ...
                    f=this.finalizeMouseCoords(stack[i], m);
                    ret=this.objects[i].dispatchEvent("mouseout", {target:this.objects[i], x:f.x, y:f.y, raw:raw});
                    if (ret === false)break;
                }
            }
        }
    ...
    this.checkMouseDown=function (event) {
        ...
        m.x+=this.viewport.x;
        m.y+=this.viewport.y;
        var stack=this.getObjectsStackByCoord(m.x, m.y, this.pixelMouseDownEvent);
        var ret, f;
        if (stack.length > 0) {
            stack=this.sortStack(stack);
            for (var i=0; i < stack.length; i++) {
                f=this.finalizeMouseCoords(stack[i], m);
                ret=stack[i].dispatchEvent("mousedown", {target:stack[i], x:f.x, y:f.y, raw:raw});
                if (ret === false)return;
            }
        }
    };
    this.checkMouseUp=function (event) {
        ...
        m.x+=this.viewport.x;
        m.y+=this.viewport.y;
        var stack=this.getObjectsStackByCoord(m.x, m.y, this.pixelMouseUpEvent, true);
        var ret, f;
        if (stack.length > 0) {
            stack=this.sortStack(stack);
            for (var i=0; i < stack.length; i++) {
                f=this.finalizeMouseCoords(stack[i], m);
                ret=stack[i].dispatchEvent("mouseup", {target:stack[i], x:f.x, y:f.y, raw:raw});
                if (ret === false)return;
            }
        }
    };
    ...
    this.hitTest=function (obj1, obj2) {
        if (obj1.rotation == 0 && obj2.rotation == 0) {
            ...
            if (width > 0 && height > 0)return true; else return false;
        }
        else {
    ...
    this.drawRectangle=function (x, y, width, height, color, fill, opacity, ignoreViewport) {
        var cns=this.canvas;
        if (typeof opacity != 'undefined')cns.ctx.globalAlpha=opacity; else cns.ctx.globalAlpha=1;
        cns.ctx.fillStyle=color;
        cns.ctx.strokeStyle=color;
        if (!ignoreViewport) {
            x-=this.viewport.x / Utils.globalScale;
            y-=this.viewport.y / Utils.globalScale;
        }
        ...
        if (fill)cns.ctx.fillRect(x - width / 2, y - height / 2, width, height); else cns.ctx.strokeRect(x - width / 2, y - height / 2, width, height);
    };
    this.drawLine=function (x1, y1, x2, y2, width, color, opacity, ignoreViewport) {
        var cns=this.canvas;
        if (color)cns.ctx.strokeStyle=color; else cns.ctx.strokeStyle='#000';
        if (width)cns.ctx.lineWidth=width * Utils.globalScale * Utils.globalPixelScale; else cns.ctx.lineWidth=1 * Utils.globalScale * Utils.globalPixelScale;
        if (opacity)cns.ctx.globalAlpha=opacity; else cns.ctx.globalAlpha=1;
        if (!ignoreViewport) {
            x1-=this.viewport.x / Utils.globalScale;
            y1-=this.viewport.y / Utils.globalScale;
            x2-=this.viewport.x / Utils.globalScale;
            y2-=this.viewport.y / Utils.globalScale;
        }
    ...
    this.renderObject=function (cns, obj) {
        ...
        if (!obj.ignoreViewport) {
            ox-=this.viewport.x * Utils.globalPixelScale;
            oy-=this.viewport.y * Utils.globalPixelScale;
        }
        var or=obj.rotation;
        ...
        try {
            ...
            cns.ctx.drawImage(obj.bitmap, obj.layer * ow, obj.currentFrame * oh, ow, oh, ox, oy, ow, oh);
        }
        catch (e) {
    ...
    this.clearObjectAABB=function (cns, obj) {
        ...
        if (!this.fillColor)cns.ctx.clearRect((obj.history.AABB[0].x - this.viewport.x) * Utils.globalPixelScale, (obj.history.AABB[0].y - this.viewport.y) * Utils.globalPixelScale, w * Utils.globalPixelScale, h * Utils.globalPixelScale); else {
    ...
    this.drawScene=function (cns, drawStatic) {
        ...
        var inTick=false;
        if (!cns.ctx)cns.ctx=cns.getContext("2d");
        if (!this.fillColor)cns.ctx.clearRect(0, 0, this.screenWidth * Utils.globalScale * Utils.globalPixelScale, this.screenHeight * Utils.globalScale * Utils.globalPixelScale); else {
            cns.ctx.fillStyle=this.fillColor;
        ...
        for (var i=0; i < this.objects.length; i++) {
            ...
            if (drawStatic && obj.static)ok=true;
            if (!inTick && !drawStatic) {
                if ((i == 0 && this.inTickIndex < obj.zIndex) || this.inTickIndex >= obj.zIndex) {
                    inTick=true;
                    this.dispatchEvent("intick");
                }
            }
            if (ok) {
                if (obj.destroy) {
    ...
    function render() {
        ...
        if (self.partialUpdate)self.drawScenePartial(self.canvas); else self.drawScene(self.canvas, false);
        if (self.showFPS) {
    ...
    this.storeTouchCoords=function (event) {
        ...
        if (e.touches)e=e.touches[0];
        if (!e)return;
        this.lastTouchCoords.x=e.pageX;
        this.lastTouchCoords.y=e.pageY;
    }
    this.checkTouchEnd=function () {
        var e={pageX:this.lastTouchCoords.x, pageY:this.lastTouchCoords.y};
        this.checkMouseUp(e);
    }
    if ("ontouchstart"in this.canvas) {
        this.canvas.ontouchstart=function (event) {
            this.renderController.storeTouchCoords(event);
...
var AssetsLibrary=function (path, scale, assets) {
    ...
    this.getSprite=function (name, params) {
        ...
        try {
            ...
            mc=new Sprite(asset.bitmap, asset.width, asset.height, asset.frames, asset.layers);
        }
        catch (e) {
...
function box2DCreateWorld(gravity) {
    ...
    if (gravity)g=new b2Vec2(0, gravity); else g=new b2Vec2(0, 300);
...
Features.prototype={set_referenceFace:function (value) {
...
Object.extend(b2CircleShape.prototype, {TestPoint:function (p) {
    var d=new b2Vec2();
    d.SetV(p);
...
b2MassData.prototype={mass:0.0, center:new b2Vec2(0, 0), I:0.0, initialize:function () {
...
Object.extend(b2PolyShape.prototype, {TestPoint:function (p) {
...
b2ContactSolver.prototype={initialize:function (contacts, contactCount, allocator) {
...
Object.extend(b2CircleContact.prototype, {initialize:function (s1, s2) {
    this.m_node1=new b2ContactNode();
    this.m_node2=new b2ContactNode();
    this.m_flags=0;
    if (!s1 || !s2) {
...
Object.extend(b2NullContact.prototype, {initialize:function (s1, s2) {
    this.m_node1=new b2ContactNode();
    this.m_node2=new b2ContactNode();
    this.m_flags=0;
    if (!s1 || !s2) {
...
Object.extend(b2PolyAndCircleContact.prototype, {initialize:function (s1, s2) {
    this.m_node1=new b2ContactNode();
    this.m_node2=new b2ContactNode();
    this.m_flags=0;
    if (!s1 || !s2) {
...
Object.extend(b2PolyContact.prototype, {initialize:function (s1, s2) {
    this.m_node1=new b2ContactNode();
    this.m_node2=new b2ContactNode();
    this.m_flags=0;
    if (!s1 || !s2) {
...
b2World.prototype={initialize:function (worldAABB, gravity, doSleep) {
...
Object.extend(b2DistanceJoint.prototype, {initialize:function (def) {
    this.m_node1=new b2JointNode();
    this.m_node2=new b2JointNode();
    this.m_type=def.type;
    this.m_prev=null;
...
function drawWorld(world, context) {
    ...
    for (var b=world.m_bodyList; b; b=b.m_next) {
        for (var s=b.GetShapeList(); s != null; s=s.GetNext())drawShape(s, context);
    }
}
function drawJoint(joint, context, color) {
...
var POLY=3;
var objects=[
    {name:"ball", image:"ball.png", type:1, width:38, height:38, frames:1, bodyType:CIRCLE, remove:false, fixed:false, density:0.8, restitution:0.2, friction:0.8},
    {name:"platform1", image:"platform1.png", type:2, width:250, height:24, frames:4, animDelay:4, bodyType:BOX, remove:false, fixed:true, density:2.0, restitution:0, friction:0.9},
    {name:"platform2", image:"platform2.png", type:2, width:126, height:24, frames:2, animDelay:4, bodyType:BOX, remove:false, fixed:true, density:2.0, restitution:0, friction:0.9},
    {name:"circle1", image:"circle1.png", type:0, width:50, height:50, frames:1, bodyType:CIRCLE, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8},
    {name:"circle2", image:"circle2.png", type:0, width:28, height:28, frames:1, bodyType:CIRCLE, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8},
    {name:"neostar1", image:"neostar1.png", type:0, width:74, height:74, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-26, -26],
            [26, -26],
            [26, 26],
    ...
    {name:"pentagon1", image:"pentagon1.png", type:0, width:50, height:47, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [0, -23],
            [25, -6],
            [15, 24],
    ...
    {name:"rectangle1", image:"rectangle1.png", type:0, width:251, height:27, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"rectangle2", image:"rectangle2.png", type:0, width:126, height:26, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"rectangle3", image:"rectangle3.png", type:0, width:101, height:26, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"rectangle4", image:"rectangle4.png", type:0, width:76, height:26, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"rectangle5", image:"rectangle5.png", type:0, width:51, height:26, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"rectangle6", image:"rectangle6.png", type:0, width:76, height:50, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"slippyrectangle1", image:"slippyrectangle1.png", type:0, width:250, height:25, frames:1, bodyType:BOX, remove:false, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"slippyrectangle2", image:"slippyrectangle2.png", type:0, width:125, height:25, frames:1, bodyType:BOX, remove:false, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"srectangle1", image:"srectangle1.png", type:0, width:251, height:27, frames:1, bodyType:BOX, remove:true, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"srectangle2", image:"srectangle2.png", type:0, width:126, height:26, frames:1, bodyType:BOX, remove:true, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"srectangle3", image:"srectangle3.png", type:0, width:101, height:26, frames:1, bodyType:BOX, remove:true, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"srectangle4", image:"srectangle4.png", type:0, width:76, height:26, frames:1, bodyType:BOX, remove:true, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"srectangle5", image:"srectangle5.png", type:0, width:51, height:26, frames:1, bodyType:BOX, remove:true, fixed:true, density:2.0, restitution:0, friction:1},
    {name:"square1", image:"square1.png", type:0, width:51, height:51, frames:1, bodyType:BOX, remove:true, fixed:false, density:2.0, restitution:0, friction:1},
    {name:"triangle1", image:"triangle1.png", type:0, width:49, height:41, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-24, 20],
            [0, -20],
            [24, 20]
    ...
    {name:"trapezoid1", image:"trapezoid1.png", type:0, width:52, height:51, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-26, -25],
            [26, -25],
            [14, 25],
    ...
    {name:"tetriscorner1", image:"tetriscorner1.png", type:0, width:76, height:76, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-38, 13],
            [38, 13],
            [38, 38],
    ...
    {name:"tetriscorner2", image:"tetriscorner2.png", type:0, width:51, height:51, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-25, -25],
            [0, -25],
            [0, 25],
    ...
    {name:"tetriscross1", image:"tetriscross1.png", type:0, width:76, height:76, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-12, -38],
            [12, -38],
            [12, 38],
    ...
    {name:"tetrisj1", image:"tetrisj1.png", type:0, width:51, height:76, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-25, 13],
            [25, 13],
            [25, 38],
    ...
    {name:"tetrisl1", image:"tetrisl1.png", type:0, width:51, height:76, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-25, 13],
            [25, 13],
            [25, 38],
    ...
    {name:"tetrist1", image:"tetrist1.png", type:0, width:76, height:76, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-38, -38],
            [38, -38],
            [38, -13],
    ...
    {name:"tetrist2", image:"tetrist2.png", type:0, width:76, height:51, frames:1, bodyType:POLY, remove:true, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-38, -25],
            [38, -25],
            [38, -0],
    ...
    {name:"mill", image:"mill.png", type:0, width:125, height:125, frames:1, bodyType:POLY, remove:false, fixed:false, density:0.8, restitution:0.2, friction:0.8, points:[
        [
            [-62.5, -12.5],
            [62.5, -12.5],
            [62.5, 12.5],
    ...
    {name:"jrectangle1", image:"jrectangle1.png", type:0, width:251, height:27, frames:1, bodyType:BOX, remove:false, fixed:false, density:2.0, restitution:0, friction:1, joints:[
        {type:"pivot", x:-112, y:0}
    ]},
    {name:"jrectangle2", image:"jrectangle2.png", type:0, width:126, height:26, frames:1, bodyType:BOX, remove:false, fixed:false, density:2.0, restitution:0, friction:1, joints:[
        {type:"pivot", x:-49.5, y:0}
    ]},
    {name:"jrectangle3", image:"jrectangle3.png", type:0, width:101, height:26, frames:1, bodyType:BOX, remove:false, fixed:false, density:2.0, restitution:0, friction:1, joints:[
        {type:"pivot", x:-37, y:0}
    ]},
    {name:"jrectangle4", image:"jrectangle4.png", type:0, width:76, height:26, frames:1, bodyType:BOX, remove:false, fixed:false, density:2.0, restitution:0, friction:1, joints:[
...
var levels=[
    {objects:[
        {type:"ball", x:169, y:289, rotation:0},
        {type:"platform1", x:172, y:373, rotation:0},
        {type:"rectangle6", x:168, y:335, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:110, y:275, rotation:0, density:0.3, friction:0.1},
        {type:"slippyrectangle1", x:45, y:306, rotation:0},
        {type:"platform2", x:247, y:373, rotation:0},
        {type:"square1", x:62, y:268, rotation:0},
        {type:"circle1", x:80, y:218, rotation:0, density:1},
        {type:"square1", x:157, y:268, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:105, y:142, rotation:0},
        {type:"slippyrectangle1", x:33, y:231, rotation:0},
        {type:"platform2", x:117, y:350, rotation:0},
        {type:"circle2", x:68, y:203, rotation:0},
        {type:"circle2", x:139, y:204, rotation:0},
        {type:"rectangle3", x:102, y:175, rotation:0},
        {type:"slippyrectangle1", x:308, y:349, rotation:0},
        {type:"circle2", x:209, y:321, rotation:0},
        {type:"circle2", x:300, y:322, rotation:0},
        {type:"rectangle3", x:253, y:292, rotation:0},
    ]},
    {objects:[
        {type:"slippyrectangle1", x:322, y:71, rotation:-0.3490658503988659},
        {type:"rectangle5", x:220, y:80, rotation:-0.3490658503988659},
        {type:"ball", x:261, y:60, rotation:0},
        {type:"platform2", x:89, y:348, rotation:0},
        {type:"slippyrectangle1", x:153, y:375, rotation:0},
        {type:"square1", x:185, y:336, rotation:0},
        {type:"square1", x:252, y:336, rotation:0},
        {type:"rectangle2", x:219, y:297, rotation:0},
        {type:"trapezoid1", x:215, y:258, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:82, y:195, rotation:0},
        {type:"platform2", x:112, y:376, rotation:0},
        {type:"mill", x:81, y:278, rotation:0},
        {type:"slippyrectangle2", x:257, y:106, rotation:-0.2617993877991494},
        {type:"slippyrectangle2", x:256, y:202, rotation:-0.2617993877991494},
        {type:"slippyrectangle2", x:258, y:291, rotation:-0.2617993877991494},
        {type:"rectangle5", x:215, y:89, rotation:-0.2617993877991494},
        {type:"rectangle5", x:214, y:184, rotation:-0.2617993877991494},
        {type:"rectangle5", x:216, y:271, rotation:-0.2617993877991494},
        {type:"circle1", x:263, y:65, rotation:0},
        {type:"circle1", x:263, y:161, rotation:0},
        {type:"circle1", x:264, y:249, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:220, y:66, rotation:0},
        {type:"platform2", x:198, y:374, rotation:0},
        {type:"mill", x:109, y:226, rotation:0},
        {type:"mill", x:219, y:150, rotation:0},
        {type:"square1", x:47, y:188, rotation:0},
        {type:"square1", x:170, y:189, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:99, y:125, rotation:0},
        {type:"platform2", x:192, y:341, rotation:0},
        {type:"triangle1", x:134, y:308, rotation:0},
        {type:"slippyrectangle2", x:91, y:210, rotation:0},
        {type:"jrectangle3", x:101, y:240, rotation:3.141592653589793},
        {type:"srectangle5", x:53, y:266, rotation:0},
        {type:"jrectangle3", x:86, y:159, rotation:0},
        {type:"rectangle5", x:127, y:185, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"slippyrectangle2", x:94, y:249, rotation:0},
        {type:"ball", x:98, y:160, rotation:0},
        {type:"rectangle4", x:97, y:193, rotation:0},
        {type:"rectangle5", x:131, y:221, rotation:0},
        {type:"rectangle5", x:55, y:222, rotation:0},
        {type:"platform2", x:209, y:332, rotation:0},
    ]},
    {objects:[
        {type:"ball", x:284, y:258, rotation:0},
        {type:"slippyrectangle1", x:321, y:284, rotation:-0.2617993877991494},
        {type:"platform2", x:64, y:374, rotation:0},
        {type:"rectangle6", x:226, y:268, rotation:-0.2617993877991494},
        {type:"tetrisj1", x:107, y:282, rotation:1.5707963267948966},
        {type:"square1", x:102, y:335, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:165, y:207, rotation:0},
        {type:"platform1", x:160, y:373, rotation:0},
        {type:"tetriscross1", x:171, y:321, rotation:0},
        {type:"tetrisl1", x:131, y:322, rotation:0},
        {type:"tetrist1", x:223, y:322, rotation:3.141592653589793},
        {type:"tetriscross1", x:145, y:270, rotation:0},
        {type:"tetrisl1", x:185, y:268, rotation:3.141592653589793},
    ], joints:[]},
    {objects:[
        {type:"mill", x:135, y:258, rotation:0.2617993877991494},
        {type:"ball", x:104, y:97, rotation:0},
        {type:"srectangle5", x:101, y:131, rotation:0},
        {type:"srectangle5", x:207, y:131, rotation:0},
        {type:"platform1", x:166, y:376, rotation:0},
        {type:"circle1", x:206, y:92, rotation:0},
        {type:"circle1", x:207, y:170, rotation:0},
        {type:"srectangle5", x:207, y:209, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:301, y:246, rotation:0},
        {type:"platform2", x:193, y:367, rotation:0},
        {type:"slippyrectangle1", x:342, y:273, rotation:-0.3490658503988659},
        {type:"rectangle5", x:250, y:270, rotation:0},
        {type:"rectangle5", x:47, y:254, rotation:0.2617993877991494},
        {type:"slippyrectangle2", x:44, y:283, rotation:0.2617993877991494},
        {type:"circle2", x:1, y:225, rotation:0, density:2},
        {type:"rectangle5", x:99, y:269, rotation:0.2617993877991494},
    ]},
    {objects:[
        {type:"platform2", x:112, y:354, rotation:0.3490658503988659},
        {type:"ball", x:115, y:104, rotation:0},
        {type:"rectangle6", x:138, y:175, rotation:0, friction:9},
        {type:"srectangle4", x:138, y:213, rotation:0},
        {type:"srectangle4", x:139, y:136, rotation:0},
        {type:"srectangle4", x:139, y:275, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"mill", x:74, y:319, rotation:0},
        {type:"ball", x:22, y:288, rotation:0},
        {type:"srectangle5", x:140, y:183, rotation:0},
        {type:"circle1", x:140, y:67, rotation:0, density:10},
        {type:"platform2", x:245, y:376, rotation:0},
        {type:"slippyrectangle2", x:195, y:324, rotation:1.5707963267948966},
        {type:"slippyrectangle2", x:294, y:324, rotation:1.5707963267948966},
        {type:"srectangle5", x:139, y:105, rotation:0},
        {type:"pentagon1", x:112, y:284, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:24, y:201, rotation:0, density:0.1},
        {type:"platform2", x:134, y:362, rotation:0.27925268031909267},
        {type:"rectangle5", x:70, y:222, rotation:0.2617993877991494},
        {type:"slippyrectangle2", x:289, y:291, rotation:-0.2617993877991494},
        {type:"slippyrectangle2", x:29, y:238, rotation:0.2617993877991494},
        {type:"rectangle1", x:235, y:276, rotation:-0.2617993877991494},
    ]},
    {objects:[
        {type:"ball", x:300, y:101, rotation:0},
        {type:"platform2", x:250, y:374, rotation:0},
        {type:"slippyrectangle2", x:296, y:136, rotation:-0.2617993877991494},
        {type:"rectangle5", x:259, y:117, rotation:-0.2617993877991494},
        {type:"jrectangle2", x:64, y:107, rotation:3.141592653589793},
        {type:"slippyrectangle2", x:-42, y:184, rotation:0},
        {type:"rectangle6", x:22, y:146, rotation:0},
        {type:"slippyrectangle2", x:300, y:299, rotation:1.5707963267948966},
        {type:"triangle1", x:199, y:341, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:280, y:60, rotation:0},
        {type:"platform1", x:167, y:379, rotation:0},
        {type:"rectangle6", x:61, y:303, rotation:1.5707963267948966},
        {type:"rectangle6", x:264, y:302, rotation:1.5707963267948966},
        {type:"rectangle1", x:162, y:223, rotation:0},
        {type:"slippyrectangle1", x:10, y:353, rotation:0},
        {type:"slippyrectangle1", x:315, y:352, rotation:0},
        {type:"rectangle4", x:279, y:94, rotation:0},
        {type:"rectangle5", x:233, y:120, rotation:0},
        {type:"rectangle5", x:327, y:120, rotation:0},
        {type:"slippyrectangle2", x:282, y:146, rotation:0},
        {type:"rectangle5", x:61, y:252, rotation:0},
        {type:"rectangle5", x:264, y:251, rotation:0},
    ], joints:[]},
    {objects:[
        {type:"ball", x:0, y:225, rotation:0},
        {type:"platform2", x:241, y:350, rotation:0},
        {type:"slippyrectangle1", x:52, y:271, rotation:0.2617993877991494},
        {type:"rectangle5", x:74, y:249, rotation:0.2617993877991494},
        {type:"rectangle5", x:124, y:263, rotation:0.2617993877991494},
        {type:"rectangle5", x:38, y:226, rotation:1.832595714594046},
        {type:"pentagon1", x:173, y:267, rotation:0.2617993877991494},
    ]},
    {objects:[
        {type:"ball", x:42, y:138, rotation:0},
        {type:"platform2", x:210, y:376, rotation:0},
        {type:"tetriscross1", x:208, y:325, rotation:0},
        {type:"slippyrectangle2", x:68, y:227, rotation:0},
        {type:"rectangle5", x:106, y:200, rotation:0},
        {type:"rectangle5", x:30, y:200, rotation:0},
        {type:"rectangle3", x:68, y:173, rotation:0},
        {type:"circle1", x:242, y:21, rotation:0},
        {type:"tetrisj1", x:263, y:362, rotation:-1.5707963267948966},
    ], joints:[]},
    {objects:[
        {type:"ball", x:163, y:114, rotation:0},
        {type:"neostar1", x:129, y:322, rotation:0},
        {type:"neostar1", x:203, y:323, rotation:0},
        {type:"neostar1", x:164, y:269, rotation:0},
        {type:"tetriscross1", x:163, y:209, rotation:0.7853981633974483},
        {type:"platform1", x:172, y:374, rotation:0},
        {type:"pentagon1", x:163, y:164, rotation:3.141592653589793},
    ], joints:[]},
    {objects:[
        {type:"platform1", x:172, y:376, rotation:0},
        {type:"pentagon1", x:113, y:342, rotation:0},
        {type:"pentagon1", x:157, y:342, rotation:0},
        {type:"pentagon1", x:201, y:342, rotation:0},
        {type:"pentagon1", x:245, y:342, rotation:0},
        {type:"pentagon1", x:134, y:315, rotation:3.141592653589793},
        {type:"pentagon1", x:178, y:315, rotation:3.141592653589793},
        {type:"pentagon1", x:223, y:315, rotation:3.141592653589793},
        {type:"pentagon1", x:136, y:268, rotation:0},
        {type:"pentagon1", x:179, y:269, rotation:0},
        {type:"pentagon1", x:224, y:269, rotation:0},
        {type:"pentagon1", x:161, y:195, rotation:0},
        {type:"pentagon1", x:158, y:242, rotation:3.141592653589793},
        {type:"pentagon1", x:201, y:243, rotation:3.141592653589793},
        {type:"pentagon1", x:203, y:196, rotation:0},
        {type:"ball", x:183, y:164, rotation:0},
    ], joints:[]}
];
;